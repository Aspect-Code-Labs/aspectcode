...
// ─────────────────────────────────────────────────────────────────────────────
// Assistant-specific wrappers
// ─────────────────────────────────────────────────────────────────────────────

export function generateCopilotContent(
  mode: InstructionsMode,
  kbAvailable = false,
): string {
  const base = generateCanonicalContentForMode(mode, kbAvailable);
  if (mode === 'permissive' || !kbAvailable) {
    return base;
  }
  return `${base}
...
- Use \`@kb.md\` to include KB context in chat
- Ask "Show me only the lines to change" to keep edits minimal
- Reference \`@kb.md\` when asking about naming conventions
`.trim();
}

export function generateCursorContent(
  mode: InstructionsMode,
  kbAvailable = false,
): string {
  const base = generateCanonicalContentForMode(mode, kbAvailable);
  if (mode === 'permissive') {
    return `---
description: Aspect Code integration - read before multi-file edits
globs: 
alwaysApply: true
---
...
`.trim();
  ...
  const tips = kbAvailable
    ? `\n\n---\n\n## Cursor Tips\n\n- Read \`kb.md\` before using Composer for multi-file edits\n- Check the Architecture section for hub files before agent-mode refactors\n- Use "Used In (files)" in the Map section to find all usages before renaming\n`
    : '';
  return `---
description: Aspect Code integration - read before multi-file edits
globs: 
...
export function generateClaudeContent(
  mode: InstructionsMode,
  kbAvailable = false,
): string {
  const base = generateCanonicalContentForMode(mode, kbAvailable);
...
- Summarize what you found in \`kb.md\` before proposing changes
- State "This file has N dependents" when editing hubs
- Use \`@kb.md\` to reference KB in chat
`.trim();
}
